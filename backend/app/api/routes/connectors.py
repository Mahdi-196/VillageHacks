...
async def google_drive_start(current=Depends(get_current_user)):
    ...
    # Expect { authLink: "https://..." , connectionId: "...", ... }
    auth_link = resp.get("authLink")
    if not auth_link:
        raise HTTPException(status_code=502, detail=f"Failed to get authLink: {resp}")
...
async def google_drive_callback(code: str | None = Query(default=None), state: str | None = Query(default=None)):
    # Supermemory handles OAuth server-side and redirects here per redirectUrl.
    # You can render a success page client-side. We just acknowledge.
